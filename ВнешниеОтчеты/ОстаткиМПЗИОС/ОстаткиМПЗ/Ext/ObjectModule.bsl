...
Функция ПолучитьТаблицу(Счета, МесяцевДляАнализа, МесяцевБезДвижения, Период, Склады)
	...
	Запрос.УстановитьПараметр("Счет002", 	ПланыСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение);
	Запрос.УстановитьПараметр("СчетМЦ04А", 	ПланыСчетов.Хозрасчетный.игсАрендованноеИЛизинговоеИмущество);
	Запрос.УстановитьПараметр("СчетМЦ08",	ПланыСчетов.Хозрасчетный.игсБуровыеИКомплектующие);
	...
	Для Каждого Строка Из ТЗ Цикл
		...
		ИначеЕсли ТипЗнч(Строка.Наименование) = Тип("СправочникСсылка.ОсновныеСредства") И Строка.Счет = ПланыСчетов.Хозрасчетный.АрендованныеОсновныеСредства Тогда			
			ОпределитьПоследнееПеремещение(Строка, Период, МесяцевБезДвижения);				
		Иначе 	
