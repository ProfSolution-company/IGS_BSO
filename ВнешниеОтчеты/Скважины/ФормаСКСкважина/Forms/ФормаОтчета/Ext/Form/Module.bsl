...
Процедура ДействияФормыОтчетСформировать(Кнопка)
	...
	//++ СурМих  для передачи таблицы тестовых данных  клиент/сервер
	//ПутьКФайлу = "C:\Users\prudkoaa\Documents\СурМих\_РР\_ФормаСКСкважина\ТаблицаБДРДляТестаБезКовычек.mxl";
	//АдресФайла = ПрочитатьИзMXL(ПутьКФайлу);
	//-- СурМих
	
	//СформироватьОтчет(АдресФайла);
...
Функция ПолучитьТаблицуДанных(НачалоПериода, КонецПериода)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
		//|	ХозрасчетныйОстаткиИОбороты.Субконто1 КАК Субконто1,  // необходима колонка "проект" при обходе результата запроса
...
Процедура ОбновитьСписокСценариев()
	...
	Попытка 
		...
		Определения = Новый WSОпределения("http://1cd01-igs-msk1/erp_igs_dev/ws/igs.1cws?wsdl", "wsFiles", "wsP@ssw0rd1809");
...
Процедура СформироватьОтчет()
	...
	Попытка
		...
		//++ СурМих  для передачи таблицы тестовых данных  клиент/сервер
		//ТаблицаУпр = ПолучитьТаблицуИзХранилища(АдресФайла);
		//-- Сурмих
		
		//ТаблицаУпр = Новый ТаблицаЗначений;
		...
		Для Каждого СтрокаУпр Из ТаблицаУпр Цикл
			...
			Если ЗначениеЗаполнено(ЭтапУСО) Тогда
				//СтрокаУпр.Проект = ЭтапУСО.Проект;    // нет такого в БСО
	...
	Для Каждого СтрокаПроекта Из ТаблицаПроектов Цикл
		...
		//ПолеТабличногоДокумента.Вывести(Область);  // вынес отсюда
...
//++ СурМих  для передачи таблицы тестовых данных  клиент/сервер
&НаКлиенте
Функция ПрочитатьИзMXL(ПутьКФайлу)
...
Функция ПолучитьТаблицуИзХранилища(АдресХранилища)
    ...
    ИтоговаяТаблица = Построитель.Результат.Выгрузить();
