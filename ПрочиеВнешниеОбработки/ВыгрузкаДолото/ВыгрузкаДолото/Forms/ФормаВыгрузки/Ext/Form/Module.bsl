...
&НаСервере
Процедура ВыгрузитьНаСервере(Период, Организация, Проект, ПутьКФайлу)
	...
	Попытка
		...
		Если ПустаяСтрока(ДанныеПодключения.ЛогинМТО) Или ПустаяСтрока(ДанныеПодключения.ПарольМТО) Или ПустаяСтрока(ДанныеПодключения.ПутьКВебСервису) Тогда  			
			ОбщегоНазначения.СообщитьПользователю("Не заполнены данные для подключения к программе ERP МТО!");
			Возврат;
		КонецЕсли;
