...
Функция СоздатьИзменитьОперацияБух() Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст =
	// <ИГС> ПР Потылицын Г.С. #0142 10.11.25 {
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НоменклатурныеГруппыРодитель.Ссылка КАК НоменклатурнаяГруппаРодитель,
	|	НоменклатурныеГруппы.Ссылка КАК НоменклатурнаяГруппа,
	|	НГигсСубъектыПроекта.Заказчик КАК Заказчик,
	|	НГигсСубъектыПроекта.Договор КАК Договор
	|ПОМЕСТИТЬ НоменклатурныеГруппы
	|ИЗ
	|	Справочник.НоменклатурныеГруппы КАК НоменклатурныеГруппыРодитель
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НоменклатурныеГруппы.игсСубъектыПроекта КАК НГигсСубъектыПроекта
	|		ПО (НоменклатурныеГруппыРодитель.Ссылка = НГигсСубъектыПроекта.Ссылка
	|				И НГигсСубъектыПроекта.Организация = &Организация)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатурныеГруппы КАК НоменклатурныеГруппы
	|		ПО НоменклатурныеГруппыРодитель.Ссылка = НоменклатурныеГруппы.Родитель
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	НоменклатурныеГруппы.НоменклатурнаяГруппаРодитель КАК Проект,
	...
	|ПОМЕСТИТЬ Итог
	|ИЗ
	|	ХозОбороты КАК ХозОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НоменклатурныеГруппы КАК НоменклатурныеГруппы
	|		ПО (ХозОбороты.НоменклатурнаяГруппа = НоменклатурныеГруппы.НоменклатурнаяГруппа
	|				И ХозОбороты.Контрагент = НоменклатурныеГруппы.Заказчик)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	НоменклатурныеГруппы.НоменклатурнаяГруппаРодитель
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
