...
Процедура СформироватьДокументы(Команда)
	...
	// Проверка на заполненность ТЧ/наличие хотя бы одной активной строки
	НетАктивныхЭлементов = Истина;
	Для Каждого ПроектСтрока Из Объект.ТаблицаПроектов Цикл
		Если ПроектСтрока.ФлагАктивности Тогда
			НетАктивныхЭлементов = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	// Продолжение проверки на заполненность
	Если НетАктивныхЭлементов Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
		"Пожалуйста, добавьте активную строку в Табличную Часть!"
		, , "ТаблицаПроектов", "Объект");
		Возврат;
	КонецЕсли;
	
	// Заполненность настройки механизмов
	Если НазначенаНоменклатураУслуг(Объект.Организация) = Ложь Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
		"""Номенклатура услуг для реализации строительных работ"" обязательна к заполнению в ""Настройки механизмов"" для текущей Организации!");
		Возврат;	
	КонецЕсли;
...
Процедура ЗаполнитьДетальнуюТЧ()
	...
	// <ИГС> ПР Потылицын Г.С. #0103 { 
	Для Каждого ПроектСтрока Из Объект.ТаблицаПроектов Цикл
		ПроектСтрока.ДокументРеализации = ПредопределенноеЗначение("Документ.ИмпРеализацияСтроительныхРаботУслуг.ПустаяСсылка");
	КонецЦикла;
	// } </ИГС>
	...
	Пока ВыборкаСкважина.Следующий() Цикл
		...
		// <ИГС> ПР Потылицын Г.С. #0013 23.09.2025 {
		Если ПроектСтрока.ФактическаяВыручка > 0 И ПроектСтрока.ФактическиеЗатраты = 0 Тогда
			//ОбщегоНазначения.СообщитьПользователю(
			//"Для", 
			//ЭтотОбъект, "СтатьяДвиженияДенежныхСредств[" + ПроектСтрока.НомерСтроки + "].Проект");
			ОбщегоНазначения.СообщитьПользователю(
			"Предупреждение: для Проекта """ + Строка(ПроектСтрока.Проект) + """ присутствует Фактическая Выручка, но отсутствуют Фактические Затраты!", ,
			"ТаблицаПроектов[" + Строка(ПроектСтрока.НомерСтроки - 1) + "].Проект", "Объект");
		КонецЕсли;
		// } </ИГС>
	...
	// <ИГС> ПР Потылицын Г.С. #0012 {
			...
			|	И игсРасчетВыполненияПроектов.ПометкаУдаления = ЛОЖЬ
			|
			|УПОРЯДОЧИТЬ ПО
			|	Ссылка УБЫВ";
...
Функция ПолучитьТекстЗапроса(ЗапросСОтборомСкважин) 
	...
	Если ЗапросСОтборомСкважин Тогда 
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.игсНачальныеОстаткиФактическихВыручкиИСебестоимости КАК игсНачальныеОстаткиФактическихВыручкиИСебестоимости
		|			ПО Скважины.Ссылка = игсНачальныеОстаткиФактическихВыручкиИСебестоимости.Скважина
		|			ЛЕВОЕ СОЕДИНЕНИЕ ПлановыеСрезПоследних КАК ПлановыеСрезПоследних
		|			ПО Скважины.Ссылка = ПлановыеСрезПоследних.Скважина
		|		ПО ЭлементыСкважин.Родитель = Скважины.Ссылка
		|ГДЕ
		|	Скважины.игсПроектЗакрыт = ЛОЖЬ
		|	И Скважины.ЭтоГруппа = ИСТИНА
		...
		|		ПО (НоменклатурныеГруппы.Родитель = Скважины.Ссылка)
		|ГДЕ
		|	Скважины.КоличествоПодгрупп = 0
		|	И Скважины.Ссылка В(&МассивСкважин)
		|
		|СГРУППИРОВАТЬ ПО
		|	Скважины.Ссылка,
		|	НоменклатурныеГруппы.Ссылка,
...
&НаСервереБезКонтекста
Функция НазначенаНоменклатураУслуг(Организация)
	...
	Возврат ЗначениеЗаполнено(НоменклатураЗначение);
