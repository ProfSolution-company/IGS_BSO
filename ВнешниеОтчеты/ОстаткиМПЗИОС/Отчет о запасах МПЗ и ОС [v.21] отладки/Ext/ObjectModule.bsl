...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Для каждого Строка Из СписокСчетов Цикл
		Счета.Добавить(Строка.Значение);
	КонецЦикла;
	
	Таблица = ПолучитьТаблицу(Счета, МесяцевДляАнализа, МесяцевБезДвижения, Период.Дата, СкладыМас);	
...
Функция ПолучитьТаблицу(Счета, МесяцевДляАнализа, МесяцевБезДвижения, Период, Склады)
	...
	|	Амортизация.СуммаОстаток КАК Амортизация,
	|	ИГС_ОстаткиОсновныхСредствОстатки.Склад КАК Склад
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет В ИЕРАРХИИ (&Счет02), &ВидыСубконтоОсновныеСредство, ) КАК Амортизация
	|		ПО ХозрасчетныйОстатки.Субконто1 = Амортизация.Субконто1
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.игсОстаткиОСНаСкладах.Остатки(&Период, ) КАК ИГС_ОстаткиОсновныхСредствОстатки
	|		ПО ХозрасчетныйОстатки.Субконто1 = ИГС_ОстаткиОсновныхСредствОстатки.ОС
	|ГДЕ
	|	(ИГС_ОстаткиОсновныхСредствОстатки.Склад В ИЕРАРХИИ (&Склады)
	...
	|	МестонахождениеОСБухгалтерскийУчетСрезПоследних.Период КАК ДатаПоследнегоПеремещения,
	|	МестонахождениеОСБухгалтерскийУчет.МОЛ КАК ПредыдущееМОЛ,
	|	ВЫБОР
	|		КОГДА ДвиженияОС.МОЛ ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|ПОМЕСТИТЬ ВТОсновныеСредстваИтоговая
	|ИЗ
	...
	|		ПО ВТОсновныеСредства.ОсновноеСредство = МестонахождениеОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			МестонахождениеОСБухгалтерскийУчет.ОсновноеСредство) КАК ДвиженияОС
	|		ПО ВТОсновныеСредства.ОсновноеСредство = ДвиженияОС.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	...
	|				ПО МестонахождениеОСБухгалтерскийУчетСрезПоследних.Период > МестонахождениеОСБухгалтерскийУчет.Период
	|					И МестонахождениеОСБухгалтерскийУчетСрезПоследних.МОЛ <> МестонахождениеОСБухгалтерскийУчет.МОЛ
	|					И МестонахождениеОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство = МестонахождениеОСБухгалтерскийУчет.ОсновноеСредство
	|		
	|		СГРУППИРОВАТЬ ПО
	|			МестонахождениеОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство) КАК ВложенныйЗапрос
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет КАК МестонахождениеОСБухгалтерскийУчет
	|			ПО ВложенныйЗапрос.ОсновноеСредство = МестонахождениеОСБухгалтерскийУчет.ОсновноеСредство
	|				И ВложенныйЗапрос.Период = МестонахождениеОСБухгалтерскийУчет.Период
	|		ПО ВТОсновныеСредства.ОсновноеСредство = ВложенныйЗапрос.ОсновноеСредство
	...
	|ВЫБРАТЬ
	|	""ОС"",
	|	ВТОсновныеСредстваИтоговая.Организация,
	|	ВТОсновныеСредстваИтоговая.Склад,
	|	ВТОсновныеСредстваИтоговая.Счет,
	|	ВТОсновныеСредстваИтоговая.Код,
	|	ВТОсновныеСредстваИтоговая.Наименование,
	|	ВТОсновныеСредстваИтоговая.ЕИ,
	|	ВТОсновныеСредстваИтоговая.Количество,
	|	ВТОсновныеСредстваИтоговая.Сумма,
	|	ВТОсновныеСредстваИтоговая.Цена,
	|	ВТОсновныеСредстваИтоговая.ПервоначальнаяСтоимость,
	|	ВТОсновныеСредстваИтоговая.ДатаПоследнегоПеремещения,
	|	ВТОсновныеСредстваИтоговая.ПредыдущееМОЛ,
	|	ВТОсновныеСредстваИтоговая.БезДвижения,
	|	СостоянияОСОрганизаций.ДатаСостояния,
	|	ВТОсновныеСредстваИтоговая.ЗаводскойНомер,
	|	ВТОсновныеСредстваИтоговая.Местонахождение
	|ИЗ
	|	ВТОсновныеСредстваИтоговая КАК ВТОсновныеСредстваИтоговая
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОСОрганизаций КАК СостоянияОСОрганизаций
	|		ПО ВТОсновныеСредстваИтоговая.Организация = СостоянияОСОрганизаций.Организация
	|			И ВТОсновныеСредстваИтоговая.Наименование = СостоянияОСОрганизаций.ОсновноеСредство
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТОсновныеСредства
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТОсновныеСредстваИтоговая";
...
Процедура СформироватьТЗКонсервация(Период)
	...
	|	МАКСИМУМ(СобытияОСОрганизацийСрезПоследних.Событие) КАК Событие,
	|	СобытияОСОрганизацийСрезПоследних.Регистратор КАК Регистратор
	|ИЗ
	|	ВТ КАК ВТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СобытияОСОрганизаций.СрезПоследних(&Период, ) КАК СобытияОСОрганизацийСрезПоследних
	|		ПО ВТ.ОсновноеСредство = СобытияОСОрганизацийСрезПоследних.ОсновноеСредство
	|			И ВТ.Период = СобытияОСОрганизацийСрезПоследних.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ.ОсновноеСредство,
	|	ВТ.Период,
